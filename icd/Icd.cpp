...
icd::icd(const icd& p_icd)
{
  ...
  memcpy(m_data, p_icd.m_data,sizeof(long) * icdLength);
}
...
#if 0
const icd&
icd::operator=(const std::string& str)
{
  SetValueString(str);
  return *this;
}
...
icd::Log10() const
{
  ...
  if(GetSign() <= 0) 
  { 
    throw new std::string(("Cannot calculate a logarithm from a number <= 0"));
  }
  res = *this;
  res = res.Log() / LN10();
...
icd::Tangent() const
{
  ...
  { 
    throw new std::string(("Cannot calculate a tangent from an angle of 1/2 pi or 3/2 pi"));
  }
...
icd::ArcSine() const
{
  ...
  {
    throw new std::string(("Cannot calculate an arcsine from a number > 1 or < -1"));
  }
  // Save the sign
  sign = number.GetSign();
  if(sign < 0)
...
icd::AsLong() const
{
  if(!FitsInLong()) 
  {
    throw new std::string("Number: Overflow in icd");
  }
  // Calculate long
...
std::string trimright(std::string str, std::string trimthis)
{
  std::size_t pos = str.find(trimthis);

  if( pos != std::string::npos )
    {
      // If at end of string then erase it
      if( (pos + trimthis.length()) == (str.length()-1) )
	{
...
icd::AsString() const
{
  ...
  if (m_precision < 16 && strAfterPoint[m_precision] >= '5') 
  {
    ...
    {
      strAfterPoint.replace(pos, 1, "0");
      pos--;
    }
    if (pos >= 0)
    {
      // *** PROBLEM ***
      //      strAfterPoint.replace(pos, 1, (strAfterPoint[pos] + 1)); // rounding
      str[pos] = str[pos] + 1;
    }
    else
    {
      pos = str.length()-1; // Rounding past the decimal point
	...
	str.replace(pos, 1, "0");
      ...
      {
	str[pos] = str[pos] + 1;
	//        str.SetAt(pos, 1, (str[pos] + 1)); // Rounding
      }
      else
      {
        str = ("1") + str;
  ...
  {
    str += "."; // Use locale?
    str += strAfterPoint.substr(0, m_precision);
  }
  str = trimright(str, ".0");
  return str;
}
...
icd::AsSQLString(bool p_strip /*=false*/) const
{
  ...
  if(str.length() > maxprec)
  {
    str = str.substr(0, maxprec);
  }
  return str;
}
#endif
...
{
  std::string display = AsString();
  std::string decpart;
  std::string intpart;
  std::string sgnpart;

  static std::string sgn(("-+"));
  ...
  if((decpos = display.find(("."))) == -1) 
    ...
    {
      intpart = display.substr(0, decpos);
      decpart = display.substr(decpos + 1, display.length()-(decpos+1));
    }
  
  while(!intpart.empty() && sgn.find(intpart.substr(0, 1)) != -1)
    {
      sgnpart += intpart.substr(0, 1);
  ...
  {
    display = (",") + intpart.substr(intpart.length()-3, 3) + display;
...
icd::AsNumeric(SQL_NUMERIC_STRUCT* p_numeric) const
{
  ...
  if(accu >= one)
  {
    throw std::string(("ICD to SQL_NUMERIC overflow!"));
  }
}
#endif
...
icd::SetLengthAndPrecision(int length, int precision)
{
  ...
  {
    throw std::string(("Decimal number (icd): The length must be between 1 and ")) + LongToString(icdLength * icdDigits);
  }
  if (precision < 0 || precision > icdPointPosition * icdDigits)
  {
    throw std::string(("Decimal number (icd): The precision must be between 0 and ")) + LongToString(icdPointPosition * icdDigits);
  }
  if (length < precision)
  {
    throw new std::string(("Decimal number (icd): The precision must be smaller than or equal to the length"));
  }
  if ((length - precision) > (icdPointPosition * icdDigits))
  {
    throw std::string(("Decimal number (icd): Number of digits before the decimal point cannot be greater than ")) + LongToString(icdPointPosition * icdDigits);
  }
  ...
  for (long i = icdLength - 1; i > highestIndex ; i--)
  {
    if (m_data[i] != 0)
    {
      throw new std::string(("Decimal number too big (ICD Overflow)"));
    }
  }
...
icd::RoundDecimals(long decimal, int precsion)
{
  ...
  if (!strDecimal.empty())
  {
    int intLength = strDecimal.length() - 1;
    for (int idx = 0; idx <= intLength; idx++)
    {
      if (idx == precsion-1)
      {
        if (idx+1 <= intLength)
        {
          ...
          strDigit += strDecimal[idx+1];
          int digit = StringToLong(strDigit);

          strTarget += strDecimal[idx];
...
void icd::dump() const
{
  printf("\nPrecision:%d", m_precision);
  
  for(int i=0; i<icdLength; i++)
    {
      printf("\n%d: %lu", i, this->m_data[i]);
    }
}
...
icd::Div(const icd& p_icd) const
{
  ...
  if (p_icd.IsNull())
  {
    throw new std::string(("Decimal number (icd): Division by zero"));
  }
...
//
void
icd::SetValueString(std::string p_value)
{
  ...
  size_t found;
  while( (found = p_value.find(",")) != std::string::npos )
    {
      p_value.erase(found, 1);
    }
  
  //p_value.Remove(',');

  // Special cases: looking for our own constants: PI / LN2 / LN10
  if(p_value.compare(("PI")) == 0)
  {
    *this = PI();
    return;
  }
  if(p_value.compare(("LN2")) == 0)
  {
    *this = LN2();
    return;
  }
  if(p_value.compare(("LN10")) == 0)
  {
    *this = LN10();
    return;
  }
  ...
  for(ptr = next - 1; index < icdLength, ptr >= from; --ptr)
  {
    ...
    {
      // ICD Overflow
      throw new std::string(("ICD Overflow"));
    }
...
//   {
//     // Cannot convert
//     throw new std::string("Cannot convert to icd");
//   }
}
...
icd::MakeEmpty()
{
  ...
  m_precision =  icdLength * icdDigits / 2;
...
icd::Mult10()
{
  ...
  if(carry)
  {
    ...
    throw new std::string(("Internal: ICD Overflow in sub operation"));
  }
}
...
long
icd::StringToLong(std::string& p_string) const
{
#if 0
  return _ttoi(p_string);
#else
  return( std::stol(p_string, nullptr, 10));
#endif
}
