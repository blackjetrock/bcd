...
static const short icdPrecision     = 104;         // Maximum allowed precision in ODBC
#endif
...
public:
  ...
  //  const icd& operator=( const std::string& str);
  const icd& operator+=(const icd& icd);
  const icd& operator-=(const icd& icd);
  const icd& operator*=(const icd& icd);
  const icd& operator/=(const icd& icd);
  ...
  // Get as a SQL string 
  //std::string AsSQLString(bool p_strip = false) const;
  ...
  //  std::string trimright(std::string icd, std::string trimthis);
...
private:
  ...
  std::string LongToString(long p_number) const;
  long    StringToLong(std::string& p_string) const;
